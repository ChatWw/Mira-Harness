    ...
    <div class="global-actions">
      <div class="application-switcher">
        <el-select v-model="currentAppCode" class="application-select" :loading="loadingApps" @change="handleAppChange">
          <template #prefix><el-icon class="application-icon"><component :is="selectedApp?.icon || 'Grid'" /></el-icon></template>
...
.application-switcher { display: flex; align-items: center; }.application-select { width: 184px; }.application-icon { color: var(--cp-primary); }.application-select :deep(.el-select__wrapper) { min-height: 36px; padding: 0 10px; border: 1px solid var(--cp-border); border-radius: $radius-md; background: var(--cp-bg-elevated); box-shadow: none; transition: border-color $transition-base, box-shadow $transition-base; &:hover { border-color: var(--cp-primary); } &.is-focused { border-color: var(--cp-primary); box-shadow: 0 0 0 3px var(--cp-primary-lighter); } }.application-option { display: flex; align-items: center; gap: $spacing-sm; }.application-option .el-tag { margin-left: auto; }
...
@include media-max($breakpoint-md) { .global-header { padding: 0 $spacing-md; }.global-brand { min-width: auto; }.global-brand > span, .user-avatar > span { display: none; }.application-select { width: 140px; } }
</style>
