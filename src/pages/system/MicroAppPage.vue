...
const columns: ProTableColumn[] = [
  { label: '应用名称', prop: 'name', minWidth: 150 }, { label: '编码', prop: 'code', width: 130 }, { label: '入口路径', prop: 'url', minWidth: 180 },
  { label: '加载策略', prop: 'strategy', width: 180, slot: 'strategy' }, { label: '健康状态', prop: 'healthStatus', width: 100 }, { label: '状态', prop: 'status', width: 100, slot: 'status' }, { label: '操作', prop: 'action', width: 190, fixed: 'right', slot: 'action' },
]
const search: ProTableSearchConfig = { fields: [
  { label: '应用名称', prop: 'name', type: 'input' }, { label: '应用编码', prop: 'code', type: 'input' },
  { label: '运行模式', prop: 'integrationMode', type: 'select', options: [{ label: 'Wujie', value: 'wujie' }, { label: '受控 iframe', value: 'iframe' }] },
...
const formSchema = computed<ProFormField[]>(() => [
  ...
  { label: '入口路径', prop: 'url', type: 'input', required: true, placeholder: '/apps/example/', rules: [{ required: true, message: '请输入同源入口路径' }, { pattern: /^\//, message: '入口必须是同源绝对路径' }] },
  { label: '运行模式', prop: 'integrationMode', type: 'select', required: true, options: [{ label: 'Wujie', value: 'wujie' }, { label: '受控 iframe', value: 'iframe' }] },
  { label: '健康状态', prop: 'healthStatus', type: 'select', required: true, options: [{ label: '健康', value: 'healthy' }, { label: '降级', value: 'degraded' }, { label: '不可用', value: 'unavailable' }] },
  { label: '允许嵌入', prop: 'embedAllowed', type: 'switch' },
...
function strategyTags(row: MicroApp) { const c = row.runtimeConfig; const tags = [row.integrationMode === 'wujie' ? 'Wujie' : 'iframe', row.integrationMode === 'wujie' && c.alive && '保活', row.integrationMode === 'wujie' && c.sync && '同步路由', row.integrationMode === 'wujie' && c.fiber && '协程模式'].filter(Boolean); return tags as string[] }
...
function add() { editing.value = false; Object.assign(form, { name: '', code: '', url: '', icon: 'Grid', sort: 0, status: 'developing', integrationMode: 'iframe', healthStatus: 'healthy', embedAllowed: true, description: '' }); visible.value = true }
