...
import { DEFAULT_CONTEXT_WINDOW, DEFAULT_HARNESS_GIT_CONFIG, isModelProviderAvailable, shouldSendWithShortcut, type HarnessContextUsage, type HarnessFileReference, type HarnessGitBranch, type HarnessGitConfig, type HarnessSkill, type ModelProviderSummary, type PermissionConfig, type PermissionMode, type SendShortcut, type ThinkingLevel } from '@/config/harness'
...
const modelOptions = computed(() => props.providers.filter(isModelProviderAvailable).flatMap(provider => provider.models.filter(model => model.enabled).map(model => ({ value: `${provider.id}:${model.id}`, providerName: provider.name, modelName: model.id, reasoning: model.reasoning, contextWindow: model.contextWindow }))))
...
.selector-option--model { min-height: 46px; }.selector-option--model > span { display: flex; min-width: 0; flex-direction: column; gap: 2px; }.selector-option--model small, .selector-option--model strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.selector-option--model small { color: var(--cp-text-tertiary); font-size: 10px; font-weight: 400; }.selector-option--model strong { color: var(--cp-text); font-size: 12px; font-weight: 500; }
