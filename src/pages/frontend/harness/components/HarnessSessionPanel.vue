<template>
  <aside class="session-panel" aria-label="工作面板">
    <header class="session-panel__header">
      <div><h2>工作面板</h2><p>{{ taskStateSummary }}</p></div>
    ...
    <section v-if="activeTab === 'task'" class="session-panel__content" aria-label="当前任务">
      <div class="task-state" :class="`is-${runStatus}`" role="status" aria-live="polite"><AppIcon :name="taskStateIcon" :class="{ 'is-spinning': runStatus === 'running' || runStatus === 'rendering' }" /><span>{{ taskState }}</span></div>
      <p v-if="runStatus === 'failed' && runError" class="task-error">{{ runError }}</p>
...
import { panelSummary, panelTaskIcon, panelTaskLabel, type HarnessPanelRunStatus } from '../panelPresentation'
...
const props = withDefaults(defineProps<{
  ...
  runStatus?: HarnessPanelRunStatus
  runError?: string
  fileChanges?: HarnessFileChange[]
}>(), { runStatus: 'idle', runError: '', fileChanges: () => [] })

defineEmits<{ close: [], 'open-file-change': [change: HarnessFileChange] }>()

const activeTab = ref<'task' | 'session' | 'details'>('task')
const tabs = computed(() => [
  { id: 'task' as const, label: '当前任务', count: props.runStatus === 'running' || props.runStatus === 'rendering' ? '进行中' : props.fileChanges.length || undefined },
  { id: 'session' as const, label: '会话' },
  { id: 'details' as const, label: '执行详情', count: props.toolCalls.length || undefined },
])
const taskState = computed(() => panelTaskLabel(props.runStatus, props.fileChanges.length))
const taskStateIcon = computed(() => panelTaskIcon(props.runStatus))
const taskStateSummary = computed(() => panelSummary(props.runStatus, props.fileChanges.length, Boolean(props.runError)))
...
.task-state { display: inline-flex; align-items: center; gap: 6px; margin-bottom: 12px; color: var(--cp-text-tertiary); font-size: 12px; font-weight: 500; }.task-state.is-running, .task-state.is-rendering { color: var(--cp-primary); }.task-state.is-completed { color: var(--cp-success); }.task-state.is-failed { color: var(--cp-danger); }.task-state.is-stopped { color: var(--cp-text-secondary); }.is-spinning { animation: panel-spin 1s linear infinite; }
.task-error { margin: -3px 0 18px; overflow-wrap: anywhere; color: var(--cp-danger); font-size: 11px; line-height: 1.5; }
