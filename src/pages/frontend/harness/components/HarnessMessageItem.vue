<template>
  <article class="message" :class="[message.role, { 'is-entering': entering }]" :data-message-id="message.id" @animationend="$emit('entrance-end', message.id)">
    ...
    <template v-else>
      ...
      <section v-if="message.run && lastMessage && !streaming" class="message-result" aria-label="任务结果">
        <div><strong>任务完成</strong><span>{{ runResultSummary }}</span></div>
        <div class="message-result__actions">
          <button v-if="message.fileChanges?.length" type="button" @click="$emit('open-work-panel')">查看变更</button>
          <button v-if="message.content" type="button" @click="copyMessage">复制结果</button>
          <button type="button" @click="$emit('continue')">继续优化</button>
        </div>
      </section>
    </template>
...
const runResultSummary = computed(() => {
  const run = props.message.run
  if (!run) return ''
  const steps = run.activities.filter(activity => activity.kind !== 'plan').length
  const failed = run.activities.filter(activity => activity.status === 'failed').length
  const changes = props.message.fileChanges?.length || 0
  const result = failed ? `${failed} 项失败` : '无失败'
  return `${changes ? `已修改 ${changes} 个文件 · ` : ''}${steps ? `${steps} 步完成 · ` : ''}${result} · ${formatDuration(run.durationMs)}`
})
...
.message-result { display: flex; align-items: center; justify-content: space-between; gap: 14px; margin-top: 18px; padding-top: 12px; border-top: 1px solid var(--cp-border-light); }.message-result > div:first-child { display: grid; min-width: 0; gap: 2px; }.message-result strong { color: var(--cp-text); font-size: 12px; font-weight: 600; }.message-result span { overflow: hidden; color: var(--cp-text-tertiary); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.message-result__actions { display: flex; flex: 0 0 auto; align-items: center; gap: 2px; }.message-result__actions button { min-height: 26px; padding: 0 6px; border: 0; border-radius: $radius-sm; color: var(--cp-text-secondary); background: transparent; font: inherit; font-size: 11px; cursor: pointer; }.message-result__actions button:hover, .message-result__actions button:focus-visible { color: var(--cp-text); background: var(--cp-bg-hover); outline: none; }
