<template>
  <article class="message" :class="[message.role, { 'is-entering': entering }]" :data-message-id="message.id" @animationend="$emit('entrance-end', message.id)">
    ...
    <template v-else>
      <HarnessRunProgress v-if="message.run || streaming" :status="status" :activities="message.run?.activities || (streaming ? activeRun?.activities : [])" :subtasks="message.run?.subtasks || (streaming ? activeRun?.subtasks : [])" :completed-at="message.run?.completedAt" :duration-ms="message.run?.durationMs ?? (streaming ? activeRunElapsed : undefined)" :summary-meta="message.run ? runUsageLabel(message.run) : undefined" :progress-label="activeRunLabel" :stop="streaming ? stopSubtask : undefined" />
      <div class="message__markdown" v-html="renderedContent" @click="handleMarkdownClick" @mouseover="handleCitationHover" @mouseleave="hideCitationCard" @focusin="handleCitationHover" @focusout="hideCitationCard" />
      ...
      <section v-if="status === 'failed' || status === 'stopped'" class="message-result" :class="{ 'is-failed': status === 'failed' }" aria-label="回复未完成">
        <p>{{ status === 'failed' ? message.run?.error || '本次回复未完成，请重试。' : '本次回复已中止，已生成的内容已保留。' }}</p>
        <div class="message-result__actions">
          <button v-if="lastMessage" type="button" :disabled="busy" @click="status === 'failed' ? $emit('rerun') : $emit('continue')">{{ status === 'failed' ? '重试' : '继续回复' }}</button>
        </div>
      </section>
    </template>
    ...
    <div v-if="!streaming" class="message__toolbar">
      ...
      <button v-if="canRerun" type="button" class="message__tool-btn" aria-label="重新生成" @click="$emit('rerun')"><AppIcon name="Refresh" /><span class="message__tool-label">重新生成</span></button>
    </div>
...
import { replyStatus } from '../runPresentation'
...
const status = computed(() => replyStatus(props.message, props.streaming))
...
const renderedContent = computed(() => renderHarnessMarkdown(markdown, props.message.content, props.message.sources))
...
.message-result { display: flex; align-items: flex-start; justify-content: space-between; gap: 14px; margin-top: 18px; padding: 12px 14px; border-radius: $radius-md; background: var(--cp-bg-hover); }.message-result p { margin: 0; color: var(--cp-text-secondary); font-size: 13px; line-height: 1.6; overflow-wrap: anywhere; }.message-result.is-failed p { color: var(--cp-danger); }.message-result__actions { display: flex; flex: 0 0 auto; align-items: center; }.message-result__actions button { min-height: 26px; padding: 0 6px; border: 0; border-radius: $radius-sm; color: var(--cp-text); background: transparent; font: inherit; font-size: 12px; cursor: pointer; }.message-result__actions button:hover:not(:disabled) { background: var(--cp-bg-elevated); }.message-result__actions button:disabled { opacity: .5; cursor: default; }
.message__attachments { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 9px; }.message__toolbar { display: flex; align-items: center; gap: 4px; margin-top: 8px; opacity: 0; transition: opacity $transition-fast; }.message:hover .message__toolbar, .message:focus-within .message__toolbar { opacity: 1; }.message__time, .message__usage { color: var(--cp-text-tertiary); font-size: 11px; }.message__usage { margin-left: auto; }.message__tool-btn { display: inline-flex; align-items: center; gap: 4px; padding: 2px 6px; border: 0; border-radius: var(--cp-radius-sm, 4px); color: var(--cp-text-secondary); background: transparent; cursor: pointer; font-size: 12px; }.message__tool-btn:hover, .message__tool-btn:focus-visible { color: var(--cp-text); background: var(--cp-bg-hover); outline: none; }.message__tool-label { max-width: 0; overflow: hidden; opacity: 0; white-space: nowrap; transition: opacity $transition-fast; }.message__tool-btn:hover .message__tool-label, .message__tool-btn:focus-visible .message__tool-label { max-width: 80px; opacity: 1; }
...
@media (prefers-reduced-motion: reduce) { .message.user.is-entering { animation: none; } }
</style>
