...
> 日期：2026-09-23  
> 状态：第一阶段范围已收敛；2026-09-24 确定默认 Harness UI 使用 React，不代表迁移、React 接入或应用安装器已经实现。
...
- Electron 提供桌面能力。
- Vue + Wujie 承担基础布局和子应用承载；Vue 还负责统一设置。
- Mira-Harness 是默认应用与产品核心入口。
- Mira-Harness 的完整工作台 UI 使用 React，Harness 执行与数据仍由 Electron 侧承载。
- 界面分为前台工作应用和统一设置区。
- 本仓库只开发 Mira-Harness、桌面平台和应用接入能力；Mira Novel Studio 的业务工程独立维护，第一阶段完成接入。
- Mira Vision 在本仓库只规划未启用的接入配置，不开发业务功能；同级空目录已预留，项目以后单独建 Git 仓库。
- 应用由平台清单管理，不开放用户自由录入微应用。
- 设置只维护白天 / 黑夜，不开放布局、主题色和加载样式配置。
- 删除当前无实际价值的菜单管理和多页签能力。
- React 是 Harness UI 的既定技术路线；开源 React 组件仅作为实现候选，不直接 Fork 其他产品的运行时。
...
- 三个同级项目目录已就位；源码迁出、配置落地和删除旧代码仍须按验收关口实施。
...
- React 接入先完成一条真实任务流程，不以重写页面数量作为验收目标。
...
`PRODUCT.md` 已在 2026-09-24 同步为以 Harness 为主体的个人 Agent 桌面工作台。现有 Harness 路线图还包含不同阶段的历史描述，不能把旧完成标记当成 React 新架构已落地。
...
| 项目归属 | 承担内容 | 本仓库保留内容 |
| --- | --- | --- |
| 当前 `Mira-Harness` Git 仓库 | 桌面平台、Vue 壳与设置、公共服务、默认 Harness UI、SDK | 平台与 Harness 源码；两款外部应用的接入配置 |
| 同级空目录 `Mira-Novel-Studio`，后续独立建仓 | 小说界面、领域逻辑、应用端业务代码，独立构建与发布静态包 | 受控应用清单、版本/能力约束和开发联调入口；不保留小说应用工程 |
| 同级空目录 `Mira-Vision`，后续独立建仓 | 暂不开发；启动时间另定 | 规划未启用的接入配置；不进入应用列表或发布流程 |

现有 `src/pages/frontend/aiNovel/`、`electron/adapters/novelApi.ts`、`electron/storage/novelStore.ts` 仍在当前仓库，是迁出前的历史实现，并非目标结构已达成。迁出时先确定作品数据的唯一所有者及 SDK/存储边界，在独立项目跑通构建、旧作品读取和回退后，才移除这里的小说业务实现；不能直接删除旧代码或用户数据。同级两个空目录尚无 Git 仓库，不要在本仓库嵌套它们的 Git 工程。
...
│   └── harness-ui/       # 默认 Harness React 应用
├── packages/
...
flowchart TB
  subgraph UI[桌面界面]
    S[Vue Shell：窗口、全局入口、应用切换、统一设置]
    W[Wujie 应用宿主]
    H[Mira-Harness React 界面]
    N[Mira Novel Studio]
    S --> W
    W --> H
    W --> N
...
负责外层窗口、全局搜索、应用选择、窗口操作、昼夜主题和公共设置。默认进入 Harness，并允许切换到已安装的第一方应用。Harness 会话、项目、审批和成果导航属于 React 应用，不由 Vue Shell 直接管理。

工作台采用外层桌面窗口与中间纯白应用显示区的两层结构：外层保留系统窗口控件、全局搜索等公共入口，内层完整交给当前应用。2026-09-24 提供的 layout 截图只锁定这一容器关系，不把旧 Harness 三栏或截图里的设计工具控件当成产品规范。是否有侧栏、侧栏如何展开，均由每个应用自行设计；Shell 不为所有应用绘制常驻业务侧栏。具体设计见 [MIRA_REACT_HARNESS_STAGE1_DESIGN_2026-09-24.md](./MIRA_REACT_HARNESS_STAGE1_DESIGN_2026-09-24.md)。

壳不重复渲染子应用的编辑器工具栏、会话列表或图库。全局入口只用于切换应用及访问平台能力；应用内导航由应用自己绘制，避免出现双侧栏、双顶栏。
...
## 7. React Harness 与开源组件：路线已定，接入仍须验证
...
React 重构 UI、连接现有 Electron 运行时是确定的目标，不再把 Vue 改造列为同等候选。旧 Vue Harness 在 React 真任务链和桌面验收完成前保留回退；全量 Fork 其他产品不是当前路线。组件可按单项能力复用，不能让第三方运行时替换 Mira 的会话、权限和数据语义。
...
- [Element Plus X](https://github.com/element-plus-x/Element-Plus-X)：历史 Vue 组件对照，不是新 React 实现的候选依赖。
...
5. React 组件复用真正减少了哪些需要维护的代码？新增了哪些运行时约束？

以“发送需求 → 工具执行 → 一次确认 → 输出文件 → 查看成果 → 重新进入会话”为最小完整原型。确认设计和复用收益后再替换生产页面。

## 8. 外部子应用如何与同一个 Electron 联调

当前三个同级目录已就位，但只有 `Mira-Harness` 是 Git 仓库；两个空目录不会随 Harness 的 Git 同步。后续独立建仓后，可在同一父目录用 IDE 多根工作区打开，但外部项目不能作为本仓库的子目录或子模块。联调时只启动本仓库的 Electron 与独立 Novel Studio 所需的前端服务；Vision 未开发时不启动、构建、配置 CI 或发布。

```text
Mira/
├── Mira-Harness/       # 当前 Git 仓库：Electron、Vue Shell、Harness、外部应用接入配置
├── Mira-Novel-Studio/  # 当前为空；后续独立建仓并开发小说前端
└── Mira-Vision/        # 当前为空；后续独立建仓
```
...
4. 执行记录与成果预览都须可定位、可阅读；是否采用工作面板由 React 应用设计稿决定。
...
React 设计稿至少覆盖空态、执行中、确认、成功成果、失败恢复五种状态。旧 Vue 页面只作为功能和数据语义的核对基线，不再参与框架路线评选。
...
以下是第一阶段内的工作批次；React Harness UI 路线已确定，具体应用内设计与受控适配仍待验证。旧结构下的原型测试只作为摸底，不算新架构验收。“Harness 和 Novel Studio 完全跑通”以本节的核心任务、外部项目接入、切换、数据恢复与发布验收为界，不要求实现所有竞品功能。

阶段 0 的源码依赖、当时的候选对比及第一刀建议见 [MIRA_PHASE0_AUDIT.md](./MIRA_PHASE0_AUDIT.md)；其中 Vue/React 对照是历史调研，小说数据长期所有权仍需后续验证。
...
| 2：React Harness UI 验证 | 在两层 Shell 内评审 React 应用设计稿，再接入现有 Electron Harness 服务；旧 Vue 页保留回退 | 接入真实事件，审批、停止、成果与历史恢复可用；记录组件复用收益 |
...
| Harness UI 技术路线 | React 作为默认应用 UI，沿用 Electron Harness 运行服务 | 方向已定；真实接入和生产切换待验收 |
