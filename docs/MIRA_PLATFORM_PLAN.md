...
> 当前代码基线：`core-platform` 仓库的 `desktop-dev` 分支。  
...
用户安装一个 Mira 桌面客户端，默认进入 Mira-Harness，用自然语言完成研究、文件处理、写作、编程和自动化任务。第一阶段只交付 Mira-Harness 和 Mira Novel Studio；Mira Vision 只保留项目目录和后续扩展位置。应用共享必要的平台能力，保留各自适合的操作界面。
...
- 界面分为前台工作应用和统一设置区。
- 第一阶段只开发 Mira-Harness 与 Mira Novel Studio。
- Mira Vision 当前只创建项目目录，不开发业务功能。
...
- 当前只更新方案文档，完成后另行讨论第一步实施范围，不直接开始迁移或删除代码。

### 1.2 本文提出、尚未正式采用的建议

- 按三个项目组织：开发 `mira-harness` 与 `mira-novel-studio`，`mira-vision` 只留目录。`mira-harness` 同时承载桌面平台和默认应用，内部按模块分开；实际建仓与拆分步骤另行确定。
...
这些建议可在评审后调整。创建新仓库、改名、删除旧页面、迁移数据和替换运行时，均不由本文自动触发。
...
| GitHub 仓库 | 承担内容 | 发布产物 |
| --- | --- | --- |
| `mira-harness` | 桌面平台、Vue 壳与设置、公共服务、默认 Harness UI、SDK | Mira 桌面安装包；后续可另产出 Harness UI 包；版本化 SDK |
| `mira-novel-studio` | 小说界面、领域逻辑、业务数据定义及所需接口声明 | Mira 子应用静态包 |
| `mira-vision` | 当前仅目录占位，无业务代码、依赖或构建配置；远程建仓时机待定 | 当前无发布产物 |

保留 `mira-harness` 作为主仓库名称符合产品以 Harness 为主体的定位，无须为了工程分层立即改名。
...
当前就拆四个也可以，但会增加跨仓库联调、依赖发布和版本配套成本。平台职责应立即明确，物理拆仓库可以稍后进行。
...
这是第一阶段目标逻辑分层，不要求每个方框立即拆成进程或独立 npm 包。Vision 仅保留目录，不接入运行架构。
...
负责本领域的界面、业务流程和校验。小说的作品和章节、Harness 的会话与计划各有自己的数据模型。Vision 当前只保留目录，不开发数据模型或运行时接入。
...
全新设置页按上述职责设计，而不是继续叠加旧后台模板页面。“微应用管理”改为“应用中心”，只展示平台批准并已发布的应用。现有“选择应用”只负责切换，下载与版本管理放在应用中心。Vision 只保留代码目录，不出现在当前应用列表、设置项或下载清单中。
...
## 8. 第一阶段两个应用如何在同一个 Electron 中开发

本地可在同一父目录组织三个项目位置，用 IDE 多根工作区打开。当前只开发 Harness 与 Novel Studio，启动一套 Electron 主程序和实际需要的前端开发服务；Vision 目录不参与启动、构建、CI 或发布。

```text
workspace/
├── mira-harness/       # 启动 Electron、Vue Shell、平台服务
├── mira-novel-studio/  # 启动小说前端开发服务
└── mira-vision/        # 仅目录占位，当前不启动、不构建
```
...
第一阶段尽量保留现有模型配置、Harness 会话与小说存储，通过服务适配暴露给新 UI。拆仓库不要求同时移动数据库或用户目录。
...
以下是第一阶段内的建议工作批次，不是已批准的执行顺序。当前先同步文档，第一步做什么将在后续讨论中确定。“Harness 和 Novel Studio 完全跑通”以本节的核心任务、切换、数据恢复与发布验收为界，不要求实现所有竞品功能。

| 阶段 | 范围与交付 | 完成标准 |
| --- | --- | --- |
| 0：确定首批工作 | 基于已明确的删减范围，选择第一步目标与验收；确认仓库归属 | 首批改动边界清楚，不同时启动全部重构 |
...
| 3：本地子应用闭环 | 优先让已有小说前端作为独立构建包加载 | 同一 Electron 切换 Harness 与小说，业务数据正确，监听无重复 |
...
Vision 仅保留目录，不分配开发批次、不生成业务脚手架、不接入应用中心。必须等 Harness 与 Novel Studio 验收完成，再单独讨论是否启动以及实现哪些能力。
...
- Vision 只留目录，应用、设置和下载列表均不出现未实现入口。
...
| 项目 | 决定 | 实现状态 |
| --- | --- | --- |
| 第一阶段应用 | Mira-Harness + Mira Novel Studio | 待按新架构跑通 |
| Mira Vision | 只创建目录，两应用跑通后再讨论开发 | 本次未创建目录 |
...
本文已按最新讨论收敛范围。本次只更新文档，没有删除业务代码、创建 Vision 目录、拆仓库、安装依赖或迁移数据；下一轮再讨论第一步具体操作。
