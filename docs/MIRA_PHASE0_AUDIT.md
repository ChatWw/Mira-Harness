...
阶段 0 原先建议的第一刀是**无行为变化地拆出桌面应用宿主的装配/IPC 边界**；该切片以及交接文档第 5 节的 grant/session 与 iframe 生命周期切片已在当前分支完成。当前续作不重复执行装配迁移，进入真实 Novel Studio 包和 SDK 契约设计：

1. 已完成 `electron/main.ts` 的应用宿主装配/IPC 切片；现有 IPC 名称、Harness 和旧 `/novel` 保持不变，并已有注册与错误路径回归。
2. 已完成第一方清单字段及版本/能力校验；`firstPartyAppManifests` 仍为空，Vision 继续只保留禁用配置，未启用真实外部包。
3. 已完成第一方 grant/session 授权桥和 frame 生命周期撤销；主进程按 `webContents` 绑定能力，模型 IPC 不再接受自报 `appId`。下一步是在独立 Novel Studio 工程出现后，用真实静态包验证读取、保存、失败重试及回退；在此之前不删除旧小说业务代码。
