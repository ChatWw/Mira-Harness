...
## 计划
- 在开始一个多步骤任务前，调用 \`set_plan\` 工具提交一份简洁、可执行的计划步骤清单。
- 每步一句短标签，必要时附一行说明；步骤是待办清单，不是推理过程，不要在步骤标签里写内部思考。
- 建议不超过 6 步；可一步完成或能直接回答的任务不需要计划。
- 调用一次即可，之后不要重复提交或改动计划；执行过程中工具会反映计划进度。
