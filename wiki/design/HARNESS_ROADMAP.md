...
- 父 Agent 可主动使用 `delegate_task`、`list_tasks`、`wait_for_tasks`、`stop_tasks` 委派工作；仅交互会话可用，自动化任务保持单 Agent。
- 内置 `explorer`、`reviewer`、`tester`、`implementer` 四种固定角色和工具白名单。子任务没有 MCP、Skill、Memory、逐次授权或递归委派能力；危险命令继续永久拦截。
- 子任务继承父模型、推理强度、项目目录与 AGENTS.md，只接收父任务说明和明确附带的项目内文本文件，不继承完整对话。每父任务最多 5 个子任务；单项限制 15 分钟、32 次模型回合、12,000 字符报告。
- 项目读写锁允许只读任务并行，`tester` / `implementer` 全生命周期独占；父任务停止会级联停止子任务，父任务不能遗留跨轮后台任务。
- 会话持久化公开状态、角色、活动、报告、错误和用量；重启后遗留任务标为 `interrupted`。前端默认折叠展示父子任务、成本和报告，不展示内部思维。
...
1. **版本收敛与 Electron 验收**：补齐父子任务取消、超时、异常退出等运行链路测试，并在 macOS Electron 中验收模型配置、权限、计划确认、子任务、自动化冲突、记忆和引用跳转。
2. **通用 Slash 注册机制**：将当前固定菜单抽象为可注册、可补全、可校验和可审计的命令框架。
3. **P2 能力**：自动化二期、项目级记忆治理、`systemMemory` 数据源、来源会话跳转等按实际使用需求排期。

P1-2 自动化一期、P1-3 记忆单阶段可靠化和网页来源引用均已完成。Consolidation、系统记忆、项目治理和自动化二期转入 P2。
