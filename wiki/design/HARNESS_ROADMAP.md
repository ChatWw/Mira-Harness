...
> 基线：基于最新代码 `desktop-dev @ 145c913`，里程碑一（P0 可靠性与工具边界）与后续 P0 批次（成本/用量、Skill、健壮性、工程健康）已完成；当前进入 P1（能力扩展）
...
**第一步：计划展示（低层 Agent 上实现，无需 AgentHarness）**

- 让 Mira 在开始任务前输出一组结构化计划步骤，以可折叠、可更新状态的 todo 活动展示。
- 实现：系统提示词引入「计划」结构；订阅模型输出并解析为 todo 项；将 todo 状态合并进 `HarnessRunActivity` / 会话消息。
- 前端：消息内「计划」区块，渲染 todo 列表、完成状态、折叠。
...
**现状**：全局 + 项目记忆已落地（`FileMemoryStore`、记忆工具、自动保存）。`systemMemory` 字段在系统提示词已预留但无数据源；记忆条目不可编辑 / 不可视化；自动记忆写入策略相对简单。
