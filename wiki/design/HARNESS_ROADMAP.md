...
> 基线：基于最新代码 `desktop-dev`。里程碑一（P0 可靠性与工具边界）与后续 P0 批次（成本/用量、Skill、健壮性、工程健康）已完成，P1-2 自动化一期已完成；当前剩余 P1-1 第二步子任务委派评估、P1-3 记忆增强，以及里程碑二 #4 的通用 Slash 注册机制。
...
**现状**：全局 + 项目记忆已落地（`FileMemoryStore`、记忆工具、自动保存）。已补充 Slash 菜单的项目记忆入口：当前会话有完整问答时复用长期记忆提炼流程并写入可见工具记录；没有完整问答时仅写入发送提示词，由后续消息走既有记忆工具。`systemMemory` 字段在系统提示词已预留但无数据源；记忆条目不可编辑 / 不可视化；自动记忆写入策略相对简单。

**设计**：

1. **系统记忆作用域**：定义应用级固定事实的来源与更新策略，注入 `systemMemory`；仅存在时注入。
2. **记忆条目可视化 / 编辑**：新增界面列出全局 / 项目记忆条目，支持查看详情、编辑、删除、来源追踪。
3. **自动记忆写入策略**：频控（同会话限次）、阈值（对话长度 / 重要度）、撤回（错误记忆可删除）；每次写入仍产生可见工具记录并过滤敏感信息。

**涉及文件**：`electron/fileMemoryStore.ts`（系统记忆作用域 + 条目来源字段）、`src/config/harness.ts`（`MemoryScope`、记忆条目类型）、`electron/harnessRuntime.ts`（`systemMemory` 注入）、系统提示词、新增记忆管理页或并入个性化页。

**验收**：用户可追溯每条记忆来源并删除；系统记忆只在存在时注入；自动写入可撤回。

**测试**：系统记忆读写单测、条目编辑 / 删除单测、自动写入频控与敏感过滤单测。
...
1. **P1-2 自动化一期** → 复用已稳定的 `HarnessRuntime`，自包含、价值直接、不踩 AgentHarness，建议先做。
2. **P1-1 计划与子任务** → 先做「计划展示」（低层 `Agent` 上、无迁移风险）；到「子任务委派」前**重读 ADR**，这是唯一需要重启 AgentHarness 评估的关卡。
3. **P1-3 记忆增强** → 小、可并行，补系统记忆 / 条目可视化 / 写入策略。

其中 P1-3 可与 P1-2 并行；P1-1 第一步可与 P1-2 交错进行，第二步需在 ADR 评估后定。
